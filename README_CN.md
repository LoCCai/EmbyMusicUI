...
设置抽屉按五组独立组织：背景可选黑色、白色或专辑封面高斯模糊；界面可选歌词居左、歌词居中、大唱片或上下布局；频谱形态可选标准频谱、峰值回落、曲线、折线或小球；频谱色彩可选纯色、双色渐变、多色渐变或彩虹渐变；歌词可独立调整主题、对齐与 70%–170% 字号。切换某一组不会暗中改变其他组。

频谱固定绘制在播放器底部的横向 Canvas 带中，不创建 Web Audio 音频链路，也不改变 Emby 的声音输出。窄幅、宽幅、全宽预设用于快速选择，展示宽度、展示高度和 30%–180% 波动幅度还可继续用滑杆精调；纯色/渐变的自定义色使用安全的十六进制文本输入和色块预览，不调用曾在部分浏览器中卡死的原生颜色选择器。所有选择分别保存在当前浏览器的 `localStorage`，重新进入歌词页时恢复。
...
或在 Windows 使用 `plugin\scripts\verify.ps1`，同时验证 C# 配置核心、API 契约、前后端字段同步、插件缺失回退、歌词合并、安全文本渲染、自有播放/音量控制转发与进度跳转、静音恢复、注音与封面旋转开关、三背景、四套界面、五种频谱形态、四种频谱色彩、五种歌词主题、各项浏览器持久化、页面恢复、歌词行状态、累积高亮、快进/后退、暂停识别、动画帧插值与 800ms 超时停止。真实 `MediaBrowser.*` 包编译和 Emby 容器加载仍必须在线完成。
