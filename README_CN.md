...
- 在歌词页注入独立音乐播放器面板：歌曲/歌手/封面、进度拖动、上一首、播放暂停、下一首、随机、循环和队列
- 提供播放页注音开关与可选沉浸模式，并在离开歌词页后自动清理
...
打开播放页歌词后，页面底部会显示歌词增强播放器。它提供歌曲信息、封面、进度拖动、上一首、播放/暂停、下一首、随机、循环、播放队列、注音开关、主题选择和沉浸模式。
...
播放器面板的视觉层放在页面顶层，避免被 Emby 播放页的裁剪容器截断，但其可见性严格归当前歌词播放页控制。离开播放页、页面被隐藏、被其他页面覆盖或旧歌词容器脱离文档时会自动隐藏，并同步退出沉浸样式；再次打开时恢复显示，并清理可能残留的重复面板。
...
或在 Windows 使用 `plugin\scripts\verify.ps1`，同时验证 C# 配置核心、API 契约、前后端字段同步、插件缺失回退、歌词合并、安全文本渲染、自有播放控件转发与进度跳转、注音/沉浸开关、五种主题、主题持久化、歌词行状态、累积高亮、快进/后退、暂停识别、动画帧插值与 800ms 超时停止。真实 `MediaBrowser.*` 包编译和 Emby 容器加载仍必须在线完成。
