...
- 接管歌词播放页的整屏视觉层，提供专辑双栏、唱片机和歌词优先三套可记忆界面
...
打开播放页歌词后，歌词增强会接管整页视觉层，不再在原界面上叠加底部控制条。播放器提供歌曲/歌手/专辑、封面氛围背景、进度拖动、音量/静音、停止、上一首、播放/暂停、下一首、随机、循环、播放队列、注音开关、封面旋转和歌词主题。

“界面”选择器提供三种布局：`专辑双栏`、`唱片机`、`歌词优先`。`唱片机`和`歌词优先`都使用圆形封面，可用“旋转”按钮随时开始或停止旋转；`专辑双栏`保持方形静止封面。界面和旋转选择分别保存在当前浏览器的 `localStorage`（键名 `emby-lyric-enhance.player-layout`、`emby-lyric-enhance.artwork-rotation`），重新进入歌词页时恢复。

这个面板负责界面与交互，实际播放、进度跳转、队列和投屏状态仍交给 Emby 原生播放内核；因此不会另起一套音频播放器，也不会绕过 Emby 的播放会话。原生按钮暂不可用时，对应自有按钮会禁用。

面板中的“样式”选择器可以即时切换以下主题：
...
整屏播放器的视觉层放在页面顶层，但其可见性严格归当前歌词播放页控制。离开播放页、切换到原生队列、页面被隐藏、被其他页面覆盖或旧歌词容器脱离文档时，会撤销页面级布局并还原 Emby 原生界面；再次打开时恢复所选布局，并清理可能残留的重复实例。
...
或在 Windows 使用 `plugin\scripts\verify.ps1`，同时验证 C# 配置核心、API 契约、前后端字段同步、插件缺失回退、歌词合并、安全文本渲染、自有播放/音量控制转发与进度跳转、注音与封面旋转开关、三套界面、五种主题、布局/旋转/主题持久化、页面恢复、歌词行状态、累积高亮、快进/后退、暂停识别、动画帧插值与 800ms 超时停止。真实 `MediaBrowser.*` 包编译和 Emby 容器加载仍必须在线完成。
