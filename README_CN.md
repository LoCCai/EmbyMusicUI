...
设置抽屉提供三种参考图布局：`专辑双栏`使用深色氛围背景、左侧方形封面和右侧歌词；`唱片机`使用浅色柔光、左侧唱盘和右侧歌词；`歌词优先`把歌词放在左侧，并在右上放置超大圆形唱片。歌曲信息位于歌词栏顶部，底部统一保留播放控制带。`唱片机`和`歌词优先`可用“旋转”按钮随时开始或停止封面旋转，`专辑双栏`保持方形静止封面。界面和旋转选择分别保存在当前浏览器的 `localStorage`（键名 `emby-lyric-enhance.player-layout`、`emby-lyric-enhance.artwork-rotation`），重新进入歌词页时恢复。

这个面板负责界面与交互，实际播放、进度跳转和队列状态仍交给 Emby 原生播放内核；因此不会另起一套音频播放器，也不会绕过 Emby 的播放会话。原生按钮暂不可用时，对应自有按钮会禁用。工具按钮使用自有提示，设置说明、歌词主题和界面布局集中在同一设置抽屉中。
