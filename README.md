...
- Stores exactly two fixed design canvases—landscape (`1920×1080`) and portrait (`1080×1920`)—with independent scale/offset transforms and a persistent out-of-stage settings safety entry. Other viewport ratios use centered contain scaling only; the design is never stretched or expanded and surplus space stays blank
...
Only orientation selects layout: landscape covers desktop, landscape tablet, and landscape phone; portrait covers portrait tablet and phone. Landscape always uses a `1920×1080` design canvas and portrait always uses `1080×1920`. Window size, ultrawide screens, short landscape screens, and soft keyboards do not create a third implicit layout. Every other ratio uses centered contain scaling with blank letterbox space; elements are never stretched and the long edge never expands the design coordinate system. Every built-in theme is a complete public V5 `fixed-canvas-v1` document; V3/V4 and the earlier V5 `anchored-canvas-v2` model remain import-compatible. Theme IDs are used only for catalog names, previews, and migration mapping, never for hidden geometry or visual CSS branches.
