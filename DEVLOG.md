...
- 新增桌面、平板、手机竖屏、手机横屏四档归一化布局，支持最近布局继承、独立覆盖恢复，以及封面、歌曲信息、歌词、频谱、进度、播放控制、音量和辅助按钮八图层编辑
- 画布编辑支持拖拽、缩放、旋转数值、层级、隐藏、锁定、对齐、吸附、键盘微调和撤销/重做；进入画布时关闭设置模态层，打开设置或按 Escape 时安全退出画布
- 设置与媒体弹层统一主题语义色，设置页保持不透明、无高斯模糊并固定在最高交互层；辅助图层隐藏时保留设置恢复入口
- 封面新增当前 Emby 图片、HTTPS URL、私有上传、焦点、适应方式与多边形裁切；修复远程字体覆盖旧上传资源、切回 Emby 封面以及歌曲元数据刷新覆盖自定义封面的分支
- 主歌词、第二行、第三行及以后分别保存字体来源、大小、字重、斜体、字距、行距、颜色、透明度、描边、阴影和辉光，并独立设置已播/当前/未播颜色与透明度
- 全部频谱分析参数、歌词显隐与跟随等待、媒体字段、弹卡样式和控制安全区纳入命名主题快照
- 新增按认证 Emby 用户隔离的 `UserWorkspace`、`Themes`、`Assets` API；主题逐文件原子写入，使用 revision 并发控制，冲突时创建副本，不设置主题数量上限
- 服务端增加严格字段白名单、精确范围、HTTPS、资源 ID、裁切、多媒体签名、单文件大小和每用户配额校验；替换资源失败时保留原文件
- 前端以 500ms 防抖自动同步草稿，保留本地离线队列；首次升级从 `DisplayPreferences.CustomPrefs` 与 `localStorage` 导入设置和最多 24 套旧主题
- 前端安装器增加构建 ID 与生成后 `lyrics.js`/`lyrics.css` SHA-256 状态核对，成对原子替换两个文件
