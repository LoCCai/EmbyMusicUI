...
- 原右上角主题选择器扩展为底部独立播放器面板，继续挂载在 `document.body`，生命周期仍严格归当前可见歌词渲染器所有
- 面板安全显示歌曲名、歌手/专辑和 Emby 主封面，不把媒体元数据写入 `innerHTML`
- 自有进度条由 `onTimeUpdate(positionTicks, runtimeTicks)` 驱动；拖动时按比例转发 `input`/`change` 给 Emby 原生 `.videoOsdPositionSlider`
- 上一首、播放/暂停、下一首、随机、循环和队列按钮转发给 Emby 4.9.5.0 的原生控件；不存在或原生禁用时自有按钮同步禁用
- 随机、循环、队列等原生 `toggleButton-active` 状态同步到面板，不复制 Emby 的播放状态机
- “注音”按钮提供当前歌词会话的第二行显隐覆盖，不删除或改变第二行逐字解析和逐字状态
- 沉浸模式只隐藏歌词页的原生冗余信息/控制视觉层；离开、被其他页面覆盖或销毁时撤销 `body` 页面类，防止污染其他页面
- 桌面和窄屏布局分别适配，并为歌词虚拟列表保留面板底部空间
- 回归测试覆盖控制转发、seek 比例、元数据安全文本、注音开关、沉浸清理、旧渲染器隔离和单实例恢复
