...
- 2026-08-09：首版播放器仅归活动歌词 `LyricsRenderer` 所有，复用现有可见性与重复实例清理规则；沉浸模式的页面级类必须由所有者在隐藏或销毁时撤销。
