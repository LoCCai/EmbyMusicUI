...
0.2.2 起，管理设置页会以“歌词增强”注册到 Emby 服务器左侧菜单。0.2.4 使用不破坏 Emby 路由页面栈的非侵入显隐管理，兼容“先执行嵌入脚本、后插入配置页”的动态时序；页面背景透明继承当前 Emby 主题。保存期间禁止重复提交，按钮和主题色状态条会显示保存进度；成功后原地重新读取服务器配置并回显确认摘要。读取失败时禁止保存空白表单，并可手动重新读取。
...
初版 0.2.0.0 已在 Windows x64、.NET SDK 8.0.423 上完成真实 Release 构建且设置页正确嵌入。随后 Emby 4.9.5.0 实机验证发现其插件加载上下文不能稳定解析同目录的独立 `EmbyLyricEnhance.Core.dll`，因此 0.2.1 起把 Core 源码直接编入主插件，只交付 `EmbyLyricEnhance.dll`。当前 0.2.4.0 已使用恢复好的官方 SDK 离线 Release 构建，结果为 0 警告、0 错误；产物不引用 Core 程序集，嵌入设置页已核对包含路由保护、主题继承和保存回读反馈。
