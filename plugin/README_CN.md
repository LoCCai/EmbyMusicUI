...
0.2.2 起，管理设置页会以“歌词增强”注册到 Emby 服务器左侧菜单。0.2.3 进一步兼容 Emby“先执行嵌入脚本、后把新配置页插入 DOM”的动态导航时序：文档级管理器监听 `pageshow`、`viewshow` 和页面插入，始终只保留并绑定最新实例。保存期间禁止重复提交，成功后原地重新读取服务器配置，不调用会触发页面导航的通用结果处理器。
...
初版 0.2.0.0 已在 Windows x64、.NET SDK 8.0.423 上完成真实 Release 构建且设置页正确嵌入。随后 Emby 4.9.5.0 实机验证发现其插件加载上下文不能稳定解析同目录的独立 `EmbyLyricEnhance.Core.dll`，因此 0.2.1 起把 Core 源码直接编入主插件，只交付 `EmbyLyricEnhance.dll`。当前 0.2.3.0 已使用恢复好的官方 SDK 离线 Release 构建，结果为 0 警告、0 错误；产物不引用 Core 程序集，嵌入设置页已核对包含延迟插入生命周期修复。
